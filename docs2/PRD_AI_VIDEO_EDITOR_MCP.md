...
**Version:** 5.0
**Date:** 2026-03-20
**Product:** ClipCannon (clipcannon.com)
**Author:** Product Team
...
|:----------|:-------------|:-----|
| **GPU** | NVIDIA GeForce RTX 5090 (32GB VRAM, 170 CUs) | Model inference, NVENC encoding, CUDA processing |
| **CPU** | AMD Ryzen 9 9950X3D (16C/32T, 5.7GHz) | FFmpeg orchestration, I/O, preprocessing |
...
1. **AI-First: The AI Is the Editor**: Every design decision answers one question: "Does the AI have what it needs to make this editing decision?" The AI is multimodal — it can read text, view images, and reason over structured data simultaneously. It receives sampled frames (actual images), word-level transcripts, and embedder analytics, all temporally aligned on the same millisecond timeline. The AI sees the video, reads what was said, and knows the quantitative signals — all at once.
2. **The AI Sees the Actual Frames**: This system is designed for Claude Code (or equivalent multimodal LLM) with a 1 million token context window and native image understanding. Sampled frames at 2fps are extracted as JPEGs and delivered to the AI alongside their timestamps. The AI sees what is happening in the video the same way a human editor would — by looking at it.
3. **Temporal Alignment Is Everything**: All data — frames, transcript words, embedder analytics — shares a unified millisecond timeline. At any timestamp, the AI can see: the frame (what is being shown), the transcript (what is being said), the speaker (who is saying it), the energy (how engaging it is), and every other signal. This temporal alignment is the core data structure of the system.
4. **Embedders Provide Quantitative Signals the AI Cannot See in Frames**: The AI can look at a frame and understand its visual content. But it cannot "hear" emotion from an image, detect laughter, measure speaking pace, or compute semantic similarity. The 12 understanding streams provide quantitative signals that augment the AI's visual perception: energy scores, speaker labels, topic clusters, reaction events, beat positions, quality metrics, and more.
5. **Provenance Hash Chain on Every Transformation**: Every data transformation — from source video to audio extraction, from audio to transcript, from frames to embeddings — is recorded with SHA-256 hashes into an immutable provenance chain. This provides full visibility into every stage of the pipeline for debugging, audit, and replay.
6. **FFmpeg Is the Renderer**: All actual video manipulation is delegated to FFmpeg. The AI produces edit decision lists (EDLs); FFmpeg executes them. The EDL is a declarative contract — the AI says *what*, FFmpeg does *how*.
7. **Platform-Native Content Strategy**: Different platforms reward different content. TikTok rewards hook-in-3-seconds pacing, YouTube rewards retention, LinkedIn rewards professionalism. Platform profiles include both technical specs AND content strategy guidance that the AI uses when making editorial decisions.
8. **Human-in-the-Loop for Publishing Only**: The AI handles all creative decisions autonomously. Humans review and approve final outputs before publishing.
9. **Stateless Tools, Stateful Projects**: Each MCP tool call is stateless. Project state (source videos, analysis results, edit plans, provenance records) persists in a project directory.
...
1. **Actual frames** (JPEG images) — the AI looks at sampled video frames directly, the same way a human editor would
2. **Transcript** (timestamped text) — the AI reads what is being said, word by word, aligned to the frames
3. **Embedder analytics** (structured JSON) — quantitative signals the AI cannot derive from looking at frames alone: energy scores, speaker IDs, topic clusters, reaction events, beat positions, quality metrics
...
**Embeddings (`.npy` files) are never sent to the AI.** They exist on disk for computational operations (similarity search, clustering, re-analysis) but the AI only receives the derived analytics and the actual frames.
...
```
STAGE 1 — READ THE SCRIPT (clipcannon_get_vud)
   ├─ The AI receives the full VUD: transcript, embedder analytics, highlights,
   │   scenes, topics, speakers, reactions, beats, quality, pacing, content safety
   ├─ This is 100% text/JSON — no images yet (~106K tokens for a 4hr video)
   ├─ The AI reads the transcript like a script
   ├─ Identifies highlights (pre-scored + reactions), topic boundaries, filler sections
   ├─ At this point the AI can make 55-60% of all editing decisions:
   │   cut plan, chapter markers, segment selection, music mood, speaker clips
   └─ The AI forms a mental model of the video's structure and content

STAGE 2 — WATCH THE OVERVIEW (clipcannon_get_storyboard)
   ├─ The AI receives storyboard grids: 48 composite images, each showing
   │   9 frames from a 30-second window, with timestamps
   ├─ ~70K tokens for complete visual coverage of a 4hr video
   ├─ The AI SEES the video — scene changes, speaker appearances, visual style,
   │   camera framing, on-screen content, audience reactions
   ├─ Cross-references with the VUD text data already in context
   ├─ Now can make 85% of all editing decisions:
   │   plus scene transitions, visual variety assessment, rough crop planning
   └─ If the user wants, the AI can produce edits at this point

STAGE 3 — INSPECT DETAILS (clipcannon_get_frame / clipcannon_get_frame_strip)
   ├─ For the remaining 15% of decisions needing close inspection:
   │   thumbnail selection, framing verification, quality spot-checks
   ├─ The AI requests specific frames by timestamp or frame strips by time range
   ├─ Typical: 30-50 additional images (~50-80K tokens)
...
**Embedding files (`.npy`) are never sent to the AI.** They exist on disk for computational operations (cosine similarity search, k-means clustering, re-analysis). The AI receives only: frames (images), transcript (text), and derived analytics (JSON with scores, labels, timestamps).
...
##### Stage 1: Text Map (clipcannon_get_vud — no images)

The AI's first perception of the video is entirely text and numbers. This single tool call delivers everything the AI needs for 55-60% of all editing decisions:

| Data | Contents | Tokens |
|:-----|:---------|:-------|
| Transcript | Full word-level transcript with timestamps, speaker labels | ~48K (4hr @ 150 WPM) |
| Scenes | Scene boundaries, shot types, quality scores, face positions | ~8K |
| Topics | Topic labels, keywords, boundaries | ~5K |
| Emotion curve | Energy/arousal/valence per 5-second window | ~12K |
| Speakers | Speaker list + per-segment speaker assignment | ~5K |
| Reactions | Laughter/applause events with timestamps | ~3K |
| Beats | Beat grid from music stem, BPM per section | ~4K |
| Silence gaps | Natural cut points | ~2K |
| On-screen text | OCR text, slide transitions | ~5K |
| Pacing | WPM, pause ratios, labels per segment | ~4K |
| Content safety | Profanity events, content rating | ~2K |
| Highlights | Pre-scored highlight segments with reasons | ~5K |
| Provenance | Hash chain summary, models used, integrity status | ~3K |
| **Total** | | **~106K tokens** |

After this call, the AI can: identify the best highlights, find topic boundaries, plan chapter markers, decide which segments are filler, determine music mood, plan speaker-based clips, and produce a complete paper edit — all without seeing a single frame. This is exactly how professional documentary editors work: transcript first, visual second.
...
**Token cost**: 80 grids × ~1,454 tokens = **~116K tokens**. Uses 80 images, reserves 20 for Stage 3.

After Stage 1 + Stage 2: ~106K + ~116K = **~222K tokens**. The AI has complete textual understanding AND adaptive visual overview. It can make 80-85% of all editing decisions.
...
| Component | Tokens | % of 1M | Images |
|:----------|:-------|:--------|:-------|
| System prompt + platform strategy | 5K | 0.5% | 0 |
| Stage 1: Text Map (VUD) | 106K | 10.6% | 0 |
| Stage 2: Visual Overview (80 adaptive grids) | 116K | 11.6% | 80 |
| Stage 3: Detail Inspection (~20 images reserved) | 32K | 3.2% | 20 |
| **Total input** | **259K** | **25.9%** | **100** |
| Available for AI reasoning + output | 741K | 74.1% | — |

**The AI uses 25.9% of its context to understand a 4-hour video.** The remaining 74.1% is available for multi-turn editing conversation, EDL generation, metadata writing, and iterative refinement. The AI can produce dozens of edits from a single video in one session.

**For shorter videos, the budget is even more favorable** — a 30-minute video uses ~40K for the VUD (shorter transcript) and the same 116K for grids (denser coverage), totaling ~190K (19%).
...
**Model**: HTDemucs / htdemucs_ft (Meta, MIT license)
**VRAM**: ~4 GB (GPU recommended, CPU fallback available)
**Speed**: ~20-30s for a 3-minute track on GPU; scales linearly with duration
**Output**: 4 WAV stems at source sample rate
...
| **Q** | Visual Quality | Frames (2fps) | BRISQUE (CPU) + DOVER-Mobile | Quality score 0-100 per frame | Filter out blurry/noisy/shaky segments |
...
| **A** | Acoustic | Full mix + stems | Librosa + madmom | Energy/RMS/spectral + beat grid | Silence, music sections, volume, beat positions |
| **C** | Chronemic | Derived from T,K,A | Logic-based computation | 4-dim temporal features | Pacing, silence patterns, rhythm |

### 3.2 Stream V: Visual Semantic (Frame Analysis)

**Model**: google/siglip-so400m-patch14-384
**VRAM**: ~1.2GB
**Output**: 1152-dim embedding per frame
**Extraction Rate**: 2 frames per second (every 500ms)
...
**Pipeline**:
1. Extract frames at 2fps using FFmpeg hardware-accelerated decode (NVDEC)
2. Resize to 384x384 maintaining aspect ratio with padding
3. Run batch inference through SigLIP on GPU (batch size 32-64)
4. Store embeddings with millisecond timestamps
5. Compute inter-frame cosine similarity for scene change detection
...
**VRAM**: ~1-2GB (GPU accelerated; CPU fallback available)
**Output**: Detected text strings + bounding boxes per frame
**Extraction Rate**: 1 frame per second (every 1000ms) — lower than visual stream because text changes less frequently
**License**: Apache 2.0 (commercial OK)

**Pipeline**:
1. Sample frames at 1fps from source video
2. Run PP-OCRv5 text detection + recognition on each frame
3. Deduplicate: if text is identical to previous frame, skip (avoids redundant entries for static slides)
4. Output: list of text regions with bounding boxes, confidence scores, and timestamps
5. Detect text-change events: when on-screen text changes significantly = slide transition / new section
...
**Model**: BRISQUE (CPU, via pyiqa) + DOVER-Mobile (GPU, ~1.9GB VRAM)
**VRAM**: 0 GB (BRISQUE) + ~1.9 GB (DOVER-Mobile, optional)
**Output**: Per-frame quality score (0-100) + per-scene quality classification
**License**: Public domain (BRISQUE) + MIT (DOVER-Mobile)

**Pipeline**:
1. For each extracted frame (2fps), compute BRISQUE score (CPU, sub-millisecond)
2. Compute Laplacian variance for blur detection (CPU, sub-millisecond)
3. Optionally: run DOVER-Mobile for aesthetic + technical quality separation (GPU)
4. Classify each frame: `good` (BRISQUE > 60), `acceptable` (40-60), `poor` (< 40)
5. Aggregate per scene: `scene_quality_avg`, `worst_frame_quality`, `blur_frame_count`
...
**Model**: ResNet-50 fine-tuned on MovieShots dataset (46K shots from 7K movie trailers)
**VRAM**: ~1-2GB
**Output**: Shot type label per key frame (1 per scene)
**License**: Open source (ResNet-50 weights are MIT/Apache)
...
**Pipeline**:
1. Extract one key frame per scene (from scene detection in Stream V)
2. Run ResNet-50 shot classifier on each key frame
3. Output: shot type label + confidence per scene
...
**Model**: faster-whisper large-v3 (CTranslate2 optimized)
**VRAM**: ~3GB
**Output**: Word-level timestamps + text segments

**Why faster-whisper**: 4x faster than OpenAI Whisper with identical accuracy. CTranslate2 backend provides INT8 quantization and batched inference on CUDA. The `large-v3-turbo` variant (809M params, ~3GB VRAM) provides 8x speedup over large-v3 with minimal accuracy loss.

**For maximum timestamp precision**: WhisperX (m-bain/whisperX) adds wav2vec2 forced alignment for true millisecond-precision word timestamps and built-in speaker diarization. Standard Whisper word timestamps have ~1 second precision; WhisperX achieves millisecond precision critical for word-level caption sync and precise cut points.

**Pipeline**:
1. Extract audio track from video via FFmpeg (PCM 16kHz mono)
2. Run faster-whisper with `word_timestamps=True` (or WhisperX for millisecond precision)
3. Output: list of segments, each with `start_ms`, `end_ms`, `text`, and per-word timestamps
4. Detect language automatically (multilingual support)
5. Optional: WhisperX speaker diarization pass (supplements Stream K)
...
**VRAM**: ~0.5GB
**Output**: 768-dim embedding per transcript segment

**Pipeline**:
1. Take transcript segments from Stream T
2. Prepend task prefix: `search_document: {segment_text}`
3. Run through Nomic Embed to produce 768-dim vectors
4. L2-normalize embeddings
5. Cluster segments by cosine similarity for topic detection
...
**Model**: wav2vec2 large (fine-tuned for emotion) or facebook/hubert-large-ll60k
**VRAM**: ~1.5GB
**Output**: 1024-dim embedding + scalar energy/arousal/valence per segment

**Pipeline**:
1. Segment audio into overlapping windows (5s windows, 2.5s stride)
2. Run through emotion model on GPU
3. Extract: arousal (energy level), valence (positive/negative), dominance
4. Compute per-segment emotion trajectory
...
**VRAM**: ~0.4GB
**Output**: 512-dim X-vector per speech segment

**Pipeline**:
1. Run Voice Activity Detection (Silero VAD) to identify speech segments
2. Extract 512-dim speaker embeddings per segment
3. Cluster embeddings to identify unique speakers
4. Assign speaker labels to all transcript segments
...
**Method**: Librosa + custom DSP (no neural model required)
**VRAM**: 0GB (CPU-based)
**Output**: Per-frame acoustic features
...
**Model**: SenseVoice-Small (FunAudioLLM)
**VRAM**: ~1.1GB
**Output**: Timestamped audio event labels
**Speed**: 70ms to process 10 seconds of audio (15x faster than Whisper-Large)
**License**: Apache 2.0 (commercial OK)

**Pipeline**:
1. Feed vocal stem (from HTDemucs) to SenseVoice-Small
2. Model outputs audio event tags with timestamps: `<Laughter>`, `<Applause>`, `<BGM>`, `<Speech>`, `<Cry>`, `<Breath>`, `<Cough>`
3. Filter to editing-relevant events: laughter, applause
4. Compute confidence and duration for each event
...
**Model**: madmom DBNBeatTracker (primary) or Beat This! (state-of-the-art alternative)
**VRAM**: 0 GB (CPU-only for madmom; ~1-2GB GPU for Beat This!)
**Output**: Beat timestamps (ms), downbeat timestamps (ms), estimated BPM
**License**: BSD (madmom, commercial OK)

**Pipeline**:
1. Feed music stem (from HTDemucs) to beat tracker — if no music detected, feed full mix
2. Detect beat positions and downbeats
3. Estimate tempo (BPM) per section
4. Generate beat grid: array of millisecond timestamps where beats occur

**Why madmom over librosa.beat**: librosa's beat tracker has a documented late bias of 20-60ms, which is perceptible in cut-on-beat editing. madmom's DBNBeatTracker achieves highest alignment with human-annotated beats across genres. Same CPU-only footprint.
...
**Model**: No new model required — post-processing on existing faster-whisper output
**VRAM**: 0 GB
**Output**: Profanity word timestamps + NSFW frame flags
**License**: N/A (word list + logic)

**Pipeline**:
1. Match faster-whisper word-level transcript against a profanity word list (~4000 words, covers English profanity, slurs, sensitive terms)
2. For each match: record word, start_ms, end_ms, severity (mild/moderate/severe)
3. Optionally: run CLIP-based NSFW classifier (~20MB MLP on top of existing SigLIP embeddings) on key frames
4. Output: profanity timestamps for auto-bleep, NSFW frame flags for exclusion
...
**Method**: Logic-based computation from Streams T, K, A
**VRAM**: 0GB
**Output**: 4-dim vector per segment [pacing_wpm, pause_duration_ms, energy_variance, speaker_change_rate]

**Pipeline**:
1. From transcript: calculate words-per-minute per segment
2. From VAD: measure pause durations and silence patterns
3. From energy: compute variance of energy within segments
4. From speaker labels: track speaker change frequency
...
| Model | VRAM | Duration |
|:------|:-----|:---------|
| HTDemucs (source separation) | ~4.0 GB | Audio separation pass (runs first, unloaded after) |
| faster-whisper large-v3 (INT8) | ~3.0 GB | Transcription pass (on vocal stem) |
| SigLIP SO400M | ~1.2 GB | Frame embedding pass |
| PaddleOCR PP-OCRv5 (mobile) | ~1.5 GB | On-screen text detection |
| Nomic Embed v1.5 | ~0.5 GB | Semantic embedding pass |
| Wav2Vec2 Large Emotion | ~1.5 GB | Emotion analysis pass (on vocal stem) |
| WavLM Base Plus SV | ~0.4 GB | Speaker embedding pass (on vocal stem) |
| SenseVoice-Small | ~1.1 GB | Reaction detection (laughter/applause) |
| BRISQUE + DOVER-Mobile | ~1.9 GB | Visual quality assessment |
| ResNet-50 shot classifier | ~1.5 GB | Shot type classification |
| **Total (sequential loading)** | **~16.6 GB** | Models loaded/unloaded in groups |
| **Total (max concurrent)** | **~11.2 GB** | After HTDemucs unloaded; audio models concurrent |
| FFmpeg NVENC/NVDEC | ~0.5 GB | During rendering |
| madmom (beat detection) | 0 GB | CPU-only |
| BRISQUE (quality, CPU tier) | 0 GB | CPU-only |
| **Peak** | **~11.7 GB** | Well within 32GB budget |
...
|:-----|:-----|:-----|:-------------|
| **Full** | 24-32 GB | RTX 5090, RTX 4090, RTX 5080 | All models concurrent, parallel batch rendering, full 12-stream pipeline, ACE-Step music at full speed, all animations |
| **Standard** | 12-16 GB | RTX 4070 Ti, RTX 3090, RTX 4080 | Sequential model loading, single render stream, ACE-Step with cpu_offload, all animations |
| **Minimal** | 8 GB | RTX 4060, RTX 3070 | Whisper medium + SigLIP only, no emotion/speaker, ACE-Step with cpu_offload (slower), MIDI/DSP audio only as fallback, all animations (CPU-rendered) |
...
- faster-whisper `large-v3-turbo` (809M params, ~3GB INT8) replaces `large-v3` (1.55B, ~6GB) on Standard tier
...
**Method**:
1. Compute semantic embeddings for sliding windows (30s window, 15s stride)
2. Calculate cosine distance between adjacent windows
3. Topic boundaries = local maxima of semantic distance curve
4. Merge segments shorter than 2 minutes with their best neighbor
5. Generate chapter titles from transcript content
...
**Method**:
1. Identify and remove: long pauses, "um/uh" filler, repeated phrases, tangents
2. Preserve: key arguments, emotional peaks, narrative transitions
3. Maintain temporal ordering (no resequencing)
4. Apply J-cuts at topic transitions for smooth flow
...
**Pipeline**:
1. Use word-level timestamps from Whisper
2. Group into display chunks (3-5 words per line)
3. Apply platform-appropriate styling
4. Support word-by-word highlight animation (karaoke-style)
5. Render via FFmpeg `drawtext` or ASS subtitle burn-in
...
**Model**: ACE-Step v1.5 (MIT license, github.com/ace-step/ACE-Step)
**Architecture**: Hybrid Language Model (Qwen3-based, 0.6B-4B params) + Diffusion Transformer (3.5B params)
**VRAM**: Under 4GB with CPU offload; 8GB recommended for full speed
**Output**: 44.1kHz stereo WAV
**Max Duration**: 4+ minutes per generation
**Speed**: RTX 5090 estimated ~1.5s per minute of audio; RTX 3090: ~4.7s per minute
...
1. Search LottieFiles API with descriptive query (e.g., "thumbs up animated")
2. Download Lottie JSON to `~/.clipcannon/assets/lottie/cache/`
3. Render and composite as normal
...
**Format**: WebM VP9 with `yuva420p` pixel format (alpha channel)
**Resolution**: 1920x1080 at 30fps
**Compositing**: FFmpeg `overlay` filter handles alpha transparency natively
...
| `clipcannon_get_vud` | **Stage 1**: Retrieve the full VUD — text/JSON only, no images. Contains transcript, all embedder analytics, scenes, topics, highlights, reactions, beats, quality, pacing, content safety, provenance. This is the AI's primary understanding of the video. | `project_id`, `streams?` |
| `clipcannon_get_storyboard` | **Stage 2**: Get 80 adaptive storyboard grids covering the full video. Each grid is a 3x3 composite (9 frames) with timestamp overlays. Frame interval adapts to video length (short videos = denser coverage). Returns grids + per-cell temporal metadata from all streams. | `project_id`, `grid_count?` (default 80), `start_ms?`, `end_ms?` |
| `clipcannon_get_transcript` | Get transcript with timestamps (also included in VUD, but available separately for focused reading) | `project_id`, `format?`, `start_ms?`, `end_ms?` |
| `clipcannon_get_frame` | **Stage 3**: Get a single full-resolution frame image at a timestamp, with temporal metadata from all streams at that moment | `project_id`, `timestamp_ms` |
| `clipcannon_get_frame_strip` | **Stage 3**: Get a 3x3 grid of frames from a specific time range, with per-cell metadata. Used to preview a potential clip's visual flow. | `project_id`, `start_ms`, `end_ms`, `count?` (default 9) |
| `clipcannon_search_content` | Semantic search across video content — returns matching segments with timestamps | `project_id`, `query`, `stream?` |
...
| **Transcription** | faster-whisper (CTranslate2) + WhisperX | 4x faster, INT8 CUDA; WhisperX adds ms-precision timestamps + diarization |
...
| **Video Quality** | BRISQUE (CPU) + DOVER-Mobile (MIT) | No-reference quality scoring. Blur, noise, shake detection |
| **Shot Classification** | ResNet-50 on MovieShots | Close-up/medium/wide detection. Informs cropping strategy |
| **Beat Detection** | madmom DBNBeatTracker (BSD) | Beat/downbeat timestamps for cut-on-beat editing. CPU-only. Replaces librosa.beat |
| **Content Safety** | Word list + CLIP-NSFW classifier | Profanity timestamps for auto-bleep. NSFW frame detection |
| **Face Detection** | MediaPipe or InsightFace (ONNX) | Fast, accurate, for smart cropping |
| **Audio Processing** | Librosa + torchaudio | Feature extraction, VAD, preprocessing |
...
| **Database** | SQLite + sqlite-vec | Project metadata + vector similarity search |
...
| **CUDA Toolkit** | CUDA 13.1+ (13.2 recommended) | CUDA Tile, Green Contexts, cuFFT device API for audio DSP |
| **Precision** | INT8 default, NVFP4 on Blackwell | Model quantization for VRAM efficiency; FP4 = 70% memory reduction |

### 7.2 CUDA & GPU Acceleration Architecture

ClipCannon leverages the RTX 5090's Blackwell architecture (compute capability 12.0) and CUDA 13.1+ features for maximum throughput. All GPU features degrade gracefully on older architectures.
...
With 3 NVENC encoders, a batch of 20 clips can be rendered in ~7 minutes instead of ~20 minutes with a single encoder. NVENC operates on dedicated silicon -- it does not consume CUDA cores or interfere with model inference.

#### 7.2.2 Green Contexts (SM Partitioning)

CUDA 13.1 Green Contexts enable static partitioning of the GPU's 170 Streaming Multiprocessors (SMs) into isolated execution domains. ClipCannon uses this for deterministic workload isolation during the analysis phase:

```
RTX 5090: 170 SMs Total
┌─────────────────────────────────────────────────────┐
│ Green Context A (70% = 119 SMs)                     │
│ ► Model inference: Whisper, SigLIP, Wav2Vec2, WavLM │
│ ► Batch embedding computation                        │
│ ► Frame extraction + quality assessment               │
├─────────────────────────────────────────────────────┤
│ Green Context B (30% = 51 SMs)                      │
│ ► Audio DSP: Librosa CUDA, cuFFT spectral analysis  │
│ ► Face detection (MediaPipe/InsightFace)             │
│ ► Scene change similarity computation                │
└─────────────────────────────────────────────────────┘
│ NVENC (dedicated silicon, outside SM budget)         │
│ NVDEC (dedicated silicon, outside SM budget)         │
└─────────────────────────────────────────────────────┘
```
...
#### 7.2.3 CUDA Tile for Custom Kernels

CUDA Tile (available on Blackwell, extended to Ampere/Ada in CUDA 13.2) enables writing high-performance GPU kernels in Python via the cuTile DSL. ClipCannon uses custom Tile kernels for:

| Kernel | Purpose | Benefit vs CPU |
|:-------|:--------|:---------------|
| `frame_similarity` | Batch cosine similarity between consecutive SigLIP embeddings | 100x faster scene detection |
| `audio_energy_rms` | Windowed RMS energy computation over audio buffer | Real-time energy curve |
| `embedding_cluster` | K-means clustering of speaker X-vectors | Fast speaker diarization |
| `caption_layout` | Per-word timing calculation and line-break optimization | Instant caption generation |

**Example: Frame Similarity Kernel (cuTile Python)**:
```python
...
**Fallback**: On GPUs without CUDA Tile support, these operations fall back to PyTorch tensor ops (still GPU-accelerated, but ~2-3x slower than Tile kernels).

#### 7.2.4 cuFFT for Audio Spectral Analysis

The acoustic feature stream uses cuFFT's device API (CUDA 13.1+) for GPU-accelerated spectral analysis:
...
ClipCannon defaults:
- **Blackwell (RTX 5090)**: INT8 for all embedding models, NVFP4 available for larger models
- **Ada (RTX 4000 series)**: INT8 for all models, FP8 for compatible models
...
~/.clipcannon/
  ...
  models/                        # Downloaded model weights
    ...
    wavlm-base-plus-sv/
    silero-vad/
    ace-step-v15-turbo-rl/       # AI music generation model
    ace-step-lora-text2samples/  # Instrumental music LoRA
  soundfonts/                    # MIDI rendering SoundFonts
    GeneralUser_GS.sf2           # Default SoundFont (~30MB)
  assets/                        # Animation & overlay assets
    lottie/                      # Shipped Lottie animations (~1.4MB)
      subscribe_bounce_01.json
      fire_emoji_pop.json
      ...                        # ~28 shipped animations
      cache/                     # Downloaded from LottieFiles (opt-in)
    webm/                        # Pre-rendered WebM with alpha (~11MB)
      light_leak_warm_01.webm
      bokeh_soft_01.webm
      glitch_digital_01.webm
      ...                        # ~20 shipped assets
      custom/                    # User-imported custom assets
    transitions/                 # GL Transition shader expressions
      crosswarp.expr
      burn.expr
      swirl.expr
      ...                        # ~60 ported GLSL transitions
    fonts/                       # Bundled fonts for lower thirds/titles
      Montserrat-Bold.ttf
  ...
  projects/
    {project_id}/
      ...
      provenance.db              # SHA-256 hash chain database (SQLite)
      source/
        original.mp4             # Source video (symlink or copy)
      analysis/
        stems/                   # HTDemucs source separation output
          vocals.wav             # Isolated speech (fed to Whisper, Wav2Vec2, WavLM, SenseVoice)
          music.wav              # Isolated music (fed to madmom beat detection)
          drums.wav              # Isolated percussion
          other.wav              # Ambient noise, effects
        transcript.json          # Word-level transcript (from vocal stem)
        frames/                  # Extracted frames (2fps)
          frame_000000.jpg
          frame_000001.jpg
          ...
        embeddings/
          visual.npy             # Frame embeddings [N, 1152]
          semantic.npy           # Transcript segment embeddings [M, 768]
          emotion.npy            # Emotion embeddings [M, 1024]
          speaker.npy            # Speaker embeddings [M, 512]
        scenes.json              # Scene boundaries + shot types + quality
        speakers.json            # Speaker diarization results
        topics.json              # Topic segmentation
        highlights.json          # Detected highlights (with reaction boost)
        acoustic_features.json   # Acoustic analysis
        on_screen_text.json      # OCR text detection + slide transitions
        quality.json             # Per-scene visual quality scores
        shot_types.json          # Shot type classification per scene
        reactions.json           # Laughter/applause/reaction events
        beats.json               # Beat grid + BPM from music stem
        content_safety.json      # Profanity timestamps + content rating
        pacing.json              # Chronemic pacing analysis
        storyboards/             # 3x3 grid composites with timestamp overlays
          grid_001.jpg           # Frames from first interval
          grid_002.jpg
          ...
          grid_080.jpg           # Always 80 grids, interval adapts to video length
          grid_manifest.json     # Grid metadata (timestamp ranges, per-cell stream data)
        vud.json                 # Complete Video Understanding Document (text/JSON only — no images)
      edits/
        {edit_id}/
          edl.json               # Edit Decision List (includes audio + animation specs)
          preview.mp4            # Low-res preview
          metadata.json          # Generated titles/descriptions
          audio/                 # Generated audio assets for this edit
            background_music.wav # AI-generated or MIDI-composed music
            sfx_whoosh_0.wav     # Programmatic sound effect
            sfx_impact_1.wav     # Programmatic sound effect
            intro_jingle.wav     # MIDI-composed jingle
            final_mix.wav        # Final mixed audio (all layers)
          animations/            # Rendered animation frames for this edit
            lower_third_001/     # PyCairo-rendered PNG sequence
              frame_0000.png
              ...
            lottie_subscribe/    # rlottie-rendered PNG sequence
              frame_0000.png
              ...
      renders/
        {render_id}/
          ...
          publish_metadata.json  # Platform-specific post data
      publish/
        queue.json               # Publishing queue
...
5. TRANSCRIBE (GPU, ~60-120s — on VOCAL STEM for better accuracy)
   └─> faster-whisper large-v3 on stems/vocals.wav (not mixed audio)
   └─> Output: transcript.json with word-level timestamps
   └─> Detect language
   └─> PROVENANCE: hash(vocals.wav) + hash(transcript.json) + model_params → prov_005 [transcribe]

6. EMBED VISUAL (GPU, ~60s for 7200 frames)
   └─> SigLIP: batch inference (batch=64)
   └─> Output: visual.npy [7200, 1152]
   └─> Compute scene boundaries → scenes.json
   └─> PROVENANCE: hash(frame_manifest) + hash(visual.npy) + hash(scenes.json) → prov_006 [embed_visual]

7. OCR TEXT DETECTION (GPU, ~60s for 3600 frames at 1fps) ★ NEW
   └─> PaddleOCR PP-OCRv5 on frames sampled at 1fps
   └─> Deduplicate consecutive identical text
   └─> Detect text-change events (slide transitions)
   └─> Output: on_screen_text.json
   └─> PROVENANCE: hash(frame_manifest) + hash(on_screen_text.json) → prov_007 [detect_text]

8. QUALITY ASSESSMENT (CPU, ~5s for 7200 frames) ★ NEW
   └─> BRISQUE score per frame (sub-ms each, CPU-only)
   └─> Laplacian variance blur detection
   └─> Optional: DOVER-Mobile for aesthetic/technical quality (~1.9GB GPU)
   └─> Aggregate per scene
   └─> Output: quality.json
   └─> PROVENANCE: hash(frame_manifest) + hash(quality.json) → prov_008 [assess_quality]

9. SHOT TYPE CLASSIFICATION (GPU, ~5s for ~50 key frames) ★ NEW
   └─> ResNet-50 shot classifier on 1 key frame per scene
   └─> Output: shot_types.json (label + crop_recommendation per scene)
   └─> PROVENANCE: hash(key_frames) + hash(shot_types.json) → prov_009 [classify_shots]

10. EMBED SEMANTIC (GPU, ~10s)
    └─> Nomic Embed: embed transcript segments
    └─> Output: semantic.npy [M, 768]
    └─> Compute topic boundaries → topics.json
    └─> PROVENANCE: hash(transcript) + hash(semantic.npy) + hash(topics.json) → prov_010 [embed_semantic]

11. EMBED EMOTION (GPU, ~30s — on VOCAL STEM)
    └─> Wav2Vec2 on stems/vocals.wav: 5s windows, 2.5s stride
    └─> Output: emotion.npy [N, 1024] + emotion_curve.json
    └─> PROVENANCE: hash(vocals.wav) + hash(emotion.npy) → prov_011 [embed_emotion]

12. EMBED SPEAKERS (GPU, ~20s — on VOCAL STEM)
    └─> WavLM on stems/vocals.wav: extract X-vectors per speech segment
    └─> Cluster: identify unique speakers
    └─> Output: speaker.npy [M, 512] + speakers.json
    └─> PROVENANCE: hash(vocals.wav) + hash(speaker.npy) + hash(speakers.json) → prov_012 [embed_speakers]

13. DETECT REACTIONS (GPU, ~15s — on VOCAL STEM) ★ NEW
    └─> SenseVoice-Small on stems/vocals.wav
    └─> Detect: laughter, applause, BGM leakage
    └─> Output: reactions.json
    └─> PROVENANCE: hash(vocals.wav) + hash(reactions.json) → prov_013 [detect_reactions]

14. ACOUSTIC ANALYSIS + BEAT DETECTION (CPU, ~15s) ★ UPDATED
    └─> Librosa on full mix: RMS energy, spectral features, silence detection
    └─> madmom on music stem: beat positions, downbeats, BPM
    └─> Output: acoustic_features.json + beats.json
    └─> PROVENANCE: hash(audio + music.wav) + hash(features + beats) → prov_014 [analyze_acoustic]

15. PROFANITY DETECTION (CPU, <1s) ★ NEW
    └─> Match transcript words against profanity word list
    └─> Optional: CLIP-NSFW on key frames (uses existing SigLIP embeddings)
    └─> Output: content_safety.json
    └─> PROVENANCE: hash(transcript) + hash(content_safety.json) → prov_015 [detect_profanity]

16. CHRONEMIC COMPUTATION (CPU, <1s)
    └─> Calculate pacing, silence patterns, energy variance
    └─> Output: pacing.json
    └─> PROVENANCE: hash(transcript + speakers + acoustic) + hash(pacing.json) → prov_016 [compute_chronemic]

17. GENERATE STORYBOARD GRIDS (CPU, ~10s) ★ NEW
    └─> Composite frames into 3x3 grids with timestamp overlays
    └─> Always 80 grids, interval adapts: max(video_duration_s / 720, 0.5)
    └─> 4hr video: 80 grids at 20s intervals (720 frames)
    └─> 10min video: 80 grids at 0.83s intervals (sees every extracted frame)
    └─> Store: analysis/storyboards/grid_001.jpg through grid_080.jpg
    └─> PROVENANCE: hash(frame_manifest) + hash(grid_manifest) → prov_017 [generate_storyboards]

18. SYNTHESIZE VUD (CPU, <1s)
    └─> Merge ALL stream outputs into unified Video Understanding Document
    └─> Integrate: scenes + shot_types + quality + on_screen_text + transcript +
         topics + emotion_curve + speakers + reactions + beats + acoustic +
         pacing + content_safety + highlights
    └─> Compute highlights (energy peaks + semantic density + reaction presence + quality filter)
    └─> Compute provenance summary for VUD
    └─> Verify chain integrity across all 17 prior records
    └─> Output: vud.json (with provenance block, text/JSON only — no images in VUD)
    └─> PROVENANCE: hash(all_inputs) + hash(vud.json) → prov_018 [synthesize_vud]

TOTAL: ~5-7 minutes for a 1-hour source video on RTX 5090
       ~15-20 minutes for a 4-hour source video
       (HTDemucs adds ~60-90s per hour; scales linearly)
...
```json
{
  ...
  "output": {
    "file_path": "analysis/transcript.json",
...
| Stage | Operation ID | Input | Output | What Is Hashed |
|:------|:------------|:------|:-------|:--------------|
| **1. Ingest** | `probe` | Source video file | `project.json` (metadata) | Source file SHA-256, FFprobe metadata |
| **2. Audio Extract** | `extract_audio` | Source video | `audio_16k.wav` | Source hash, WAV hash, FFmpeg params |
| **3. Frame Extract** | `extract_frames` | Source video | `frames/frame_*.jpg` (manifest hash) | Source hash, manifest of all frame hashes, extraction rate |
| **4. Transcribe** | `transcribe` | `audio_16k.wav` | `transcript.json` | Audio hash, transcript hash, Whisper model + params |
| **5. Visual Embed** | `embed_visual` | `frames/` manifest | `visual.npy` | Frames manifest hash, embeddings hash, SigLIP model version |
| **6. Scene Detect** | `detect_scenes` | `visual.npy` | `scenes.json` | Embeddings hash, scenes hash, similarity threshold |
| **7. Semantic Embed** | `embed_semantic` | `transcript.json` | `semantic.npy` | Transcript hash, embeddings hash, Nomic model version |
| **8. Topic Detect** | `detect_topics` | `semantic.npy` | `topics.json` | Semantic hash, topics hash, clustering params |
| **9. Emotion Embed** | `embed_emotion` | `audio_16k.wav` | `emotion.npy` + `emotion_curve.json` | Audio hash, emotion hash, Wav2Vec2 model version, window params |
| **10. Speaker Embed** | `embed_speakers` | `audio_16k.wav` | `speaker.npy` + `speakers.json` | Audio hash, speaker hash, WavLM model version, cluster count |
| **11. Acoustic Analyze** | `analyze_acoustic` | `audio_16k.wav` | `acoustic_features.json` | Audio hash, features hash, Librosa params |
| **12. Chronemic Compute** | `compute_chronemic` | Transcript + speakers + acoustic | `pacing.json` | Hashes of all 3 inputs, output hash |
| **13. VUD Synthesis** | `synthesize_vud` | All analysis outputs | `vud.json` | Hash of all input hashes concatenated, VUD hash |
| **15. EDL Create** | `create_edl` | VUD + AI decision | `edl.json` | VUD hash, EDL hash, AI model ID, prompt/context hash |
| **16. Music Generate** | `generate_music` | ACE-Step prompt + params | `background_music.wav` | Prompt hash, params hash, output WAV hash, model version + seed |
| **17. SFX Generate** | `generate_sfx` | DSP parameters | `sfx_*.wav` | Params hash, output hash |
| **18. Audio Mix** | `mix_audio` | Source audio + music + SFX | `final_mix.wav` | All input hashes, mix params, output hash |
| **19. Animation Render** | `render_animation` | Animation spec (from EDL) | PNG frame sequence manifest | Spec hash, manifest hash, renderer version |
| **20. Video Render** | `render_video` | EDL + all assets | `output.mp4` | EDL hash, all asset hashes, FFmpeg command hash, output hash |
| **21. Thumbnail Generate** | `generate_thumbnail` | Source frame | `thumbnail.jpg` | Frame hash, thumbnail hash, sizing params |
| **22. Metadata Generate** | `generate_metadata` | VUD + EDL + platform | `publish_metadata.json` | VUD hash, EDL hash, platform, output hash, AI model ID |
| **23. Publish** | `publish` | Output video + metadata | Platform post ID | Video hash, metadata hash, platform, post ID, API response hash |

**Total: 23 provenance points per full pipeline run** (probe → publish). Each links to its parent(s) via chain hash.

#### 7.5.4 Provenance Storage

Provenance records are stored in a per-project SQLite database:

```
~/.clipcannon/projects/{project_id}/provenance.db
```
...
1. **Face Detection**: Run MediaPipe Face Detection on key frames (1 per scene)
2. **Face Tracking**: Interpolate face positions between key frames
3. **Crop Window**: Position the 9:16 crop window to keep primary face(s) centered
4. **Smoothing**: Apply temporal smoothing to prevent jerky crop movements
5. **Fallback**: If no faces detected, use center crop with rule-of-thirds bias

---

## 8. Publishing Pipeline

### 8.1 Platform API Integration
...
1. **Speed**: 5-minute analysis vs. hours. Render in seconds, not minutes. 3 parallel NVENC encoders
2. **Risk**: AI produces consistent quality. Human approval prevents bad posts. Performance-based billing (only charge on success)
3. **Ease**: One video in → everything handled. No editing skills required. Dashboard shows everything clearly
4. **Price**: Local GPU = zero per-API-call cost. Amortized over volume, drastically cheaper than human editors ($50-150/hour) or cloud services ($20-50/video)
...
**Charge Flow**:
1. User initiates video analysis via MCP tool
2. MCP server calls `POST /v1/charge` on local license server
3. License server deducts credits locally + verifies with Cloudflare D1 (source of truth)
4. On success: credits confirmed, processing begins
5. On failure (insufficient credits): automatic refund, processing blocked with clear message
6. On processing error: automatic refund to local + central balance
...
**Growth levers**:
1. **Volume**: More creators → more credits purchased
2. **Platform expansion**: Add more platforms (Twitter/X, Pinterest, Threads) → more renders per source
3. **Premium features**: Advanced ACE-Step LoRA fine-tuning, custom brand audio styles, premium animation packs at higher credit rates
4. **Agency tier**: Multi-seat dashboard, team management, bulk pricing
5. **Template marketplace**: User-created editing templates (audio presets, animation styles, brand kits), revenue share
6. **Animation marketplace**: Community-contributed Lottie/WebM animation packs, credit-based purchasing
...
3. Email sent via Resend API (or dev mode returns token in console)
...
1. Use ClipCannon to process YOUR OWN content about ClipCannon
2. Every demo video is also a proof-of-concept: "This video was cut into 15 clips and posted to 5 platforms by ClipCannon"
3. Technical tutorials as lead magnets
4. Open-source the MCP server, monetize through credits (freemium model)

**The 4 ways to get customers** (Hormozi):
1. **Warm outreach**: DM content creators you follow. "I built something that turns your 1-hour podcast into 20 platform-ready clips in 5 minutes. Want to try it?"
2. **Content**: YouTube/Twitter showing before/after of ClipCannon processing a real video
3. **Cold outreach**: Automated but personalized emails to creators with >10K subscribers who post long-form
4. **Paid ads**: After product-market fit, scale with paid acquisition
...
1. **Speed + Ease dominance**: No competitor offers 12-stream multimodal understanding locally. Existing tools (Opus Clip, Descript) are cloud-only, per-video priced, and don't run locally
2. **Local-first**: Data never leaves the machine. Critical for creators with unreleased content, brand deals, or NDAs
3. **GPU utilization**: Amortizes the RTX 5090 investment that creators already made for gaming/streaming. Zero marginal compute cost
4. **Embedder architecture**: The Hexa-Modal-derived 12-stream approach is novel and produces richer understanding than competitors using only transcription
5. **MCP protocol**: Plugs into any AI model, not locked to one vendor. Future-proof against model provider changes
6. **Zero-cost audio**: AI-generated music and programmatic SFX eliminate stock music licensing fees ($15-50/track at competitors). Every generated track is original, commercially safe, and tailored to the content
7. **Full creative suite**: Professional motion graphics (lower thirds, titles, transitions, CTAs) built in -- competitors like Opus Clip produce bare clips with no overlays. CapCut has templates but requires manual selection. ClipCannon's AI selects and parameterizes overlays autonomously
...
1. **Source Separation (HTDemucs)** isolates clean vocal, music, and noise stems — ensuring every downstream audio model operates on clean input rather than a mixed signal. This is the force multiplier that makes all audio analysis dramatically more accurate.

2. **Transcript + timestamps (faster-whisper on vocal stem)** gives the AI complete knowledge of *what is being said* and *when*. With source separation, transcription accuracy improves significantly on music-heavy sources.

3. **Visual embeddings + scene detection (SigLIP)** gives the AI knowledge of *what is being shown* and *when the visual context changes*.

4. **On-screen text (PaddleOCR)** gives the AI knowledge of *what text is displayed* — slide titles, URLs, names, code. Critical for tutorials, presentations, and product demos. Enables slide-based chapter detection.

5. **Visual quality (BRISQUE/DOVER)** gives the AI knowledge of *which frames are sharp and well-lit vs. blurry and shaky*. The AI never selects low-quality footage for highlights.

6. **Shot type (ResNet-50)** gives the AI knowledge of *how the camera is framed* — close-up, medium, wide. Directly drives cropping decisions for platform-specific aspect ratios.

7. **Emotion/energy (Wav2Vec2 on vocal stem)** gives the AI knowledge of the *emotional arc*. With source separation, emotion detection is not contaminated by background music energy.

8. **Speaker diarization (WavLM on vocal stem)** gives the AI knowledge of *who is speaking when*. With source separation, speaker embeddings are cleaner.

9. **Reactions (SenseVoice)** gives the AI knowledge of *where laughter, applause, and audience engagement occurs*. This is the strongest highlight signal for conversational content — a signal no other stream captures.

10. **Beat detection (madmom on music stem)** gives the AI knowledge of *where musical beats fall*. Enables professional cut-on-beat editing when music is present.

11. **Acoustic features (Librosa)** gives the AI knowledge of *silence gaps, music sections, and volume dynamics*.

12. **Content safety (profanity detection)** gives the AI knowledge of *where profanity occurs* for auto-bleep and platform-appropriate content selection.

13. **Chronemic features** give the AI knowledge of *pacing and rhythm*. Fast-paced sections indicate excitement. Slow sections may indicate reflection or filler.
